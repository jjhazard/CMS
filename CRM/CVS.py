...
####################################
#Path variables
folderPath = '/home/pi/CRM/CVSCodes/'
...
#Check the dispatched directory for the code
def validate(code):
...
####################################
def receive():
    code = Receive('/home/pi/CRM/CommPlaceholder/comms').execute() #Receive from Lora transciever
    if code: #Verify received a valid code
        Add(dispatched, code).execute()
    else:
        print("Bad Code: ", code)
def communicate():
    return os.path.exists('/home/pi/CRM/CommPlaceholder/comms') #Check for comms
